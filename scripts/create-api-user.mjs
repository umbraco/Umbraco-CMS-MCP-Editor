...
const MAX_RETRIES = parseInt(process.env.API_USER_RETRIES || "12", 10);
const RETRY_DELAY_MS = parseInt(process.env.API_USER_RETRY_DELAY || "5000", 10);
...
async function main() {
  ...
  for (let i = 1; i <= MAX_RETRIES; i++) {
    try {
      ...
      console.log(`  Attempt ${i}/${MAX_RETRIES} failed: ${err.message}`);
      console.log(`  Retrying in ${RETRY_DELAY_MS / 1000}s (Umbraco may still be installing)...`);
