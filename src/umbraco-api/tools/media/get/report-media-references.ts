...
const inputSchema = {
  id: z.string().uuid().describe("The ID of the media item to find references for"),
...
const outputSchema = z.object({
  ...
  })).describe("Items that reference this media item"),
});

const tool: ToolDefinition<typeof inputSchema, typeof outputSchema> = {
  name: "report-media-references",
  description: "List all items that reference a media item — e.g. pages with image or media pickers pointing at it, or document-type configurations using it. Use before delete-media or move-media to understand impact. Uses the management API's referenced-by index (no tree walk).",
  inputSchema,
  outputSchema,
  slices: ["read"],
