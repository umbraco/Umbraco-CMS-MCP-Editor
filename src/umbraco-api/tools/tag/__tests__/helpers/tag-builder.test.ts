/**
 * Tag Builder & Helper Tests
 *
 * Verifies the re-exported ContentBuilder and ContentTestHelper work correctly
 * for creating and cleaning up test pages used by tag tests.
 */
...
const TEST_PAGE_NAME = "_Test TagBuilder";
...
describe("TagBuilder", () => {
  ...
  afterEach(async () => {
    await TagTestHelper.cleanup(TEST_PAGE_NAME);
  }, 30000);

  it("should create a page and find it by name", async () => {
    if (!documentTypeId) return;

    await new TagBuilder()
      .withName(TEST_PAGE_NAME)
      .withDocumentType(documentTypeId)
      .create();

    const found = await TagTestHelper.findDocument(TEST_PAGE_NAME);
    expect(found).toBeDefined();
    expect(TagTestHelper.getNameFromItem(found)).toBe(TEST_PAGE_NAME);
  }, 30000);
