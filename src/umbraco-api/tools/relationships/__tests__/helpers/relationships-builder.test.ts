/**
 * Relationships Builder & Helper Tests
 *
 * Verifies the re-exported ContentBuilder and ContentTestHelper work correctly
 * for creating and cleaning up test pages used by relationships tests.
 */
...
const TEST_PAGE_NAME = "_Test RelationshipsBuilder";
...
describe("RelationshipsBuilder", () => {
  ...
  afterEach(async () => {
    await RelationshipsTestHelper.cleanup(TEST_PAGE_NAME);
  }, 30000);

  it("should create a page and find it by name", async () => {
    if (!documentTypeId) return;

    await new RelationshipsBuilder()
      .withName(TEST_PAGE_NAME)
      .withDocumentType(documentTypeId)
      .create();

    const found = await RelationshipsTestHelper.findDocument(TEST_PAGE_NAME);
    expect(found).toBeDefined();
    expect(RelationshipsTestHelper.getNameFromItem(found)).toBe(TEST_PAGE_NAME);
  }, 30000);
