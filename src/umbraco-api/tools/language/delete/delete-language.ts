...
const tool: ToolDefinition<typeof inputSchema, typeof outputSchema> = {
  ...
  handler: async ({ isoCode }, extra) => {
    ...
    if (!await confirmAction(extra, confirmMessage, { title: "Confirm delete language", defaultValue: false })) {
      return createToolResult({ message: "Delete cancelled", isoCode, name });
    }

    // Step 3: Delete the language
    const deleteResult = await mcpClientManager.callTool("cms", "delete-language", { isoCode });
