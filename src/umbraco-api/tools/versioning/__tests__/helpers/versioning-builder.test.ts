/**
 * Versioning Builder & Helper Tests
 *
 * Verifies the re-exported ContentBuilder and ContentTestHelper work correctly
 * for creating and cleaning up test pages used by versioning tests.
 */
...
const TEST_PAGE_NAME = "_Test VersioningBuilder";
...
describe("VersioningBuilder", () => {
  ...
  afterEach(async () => {
    await VersioningTestHelper.cleanup(TEST_PAGE_NAME);
  }, 30000);

  it("should create a page and find it by name", async () => {
    if (!documentTypeId) return;

    await new VersioningBuilder()
      .withName(TEST_PAGE_NAME)
      .withDocumentType(documentTypeId)
      .create();

    const found = await VersioningTestHelper.findDocument(TEST_PAGE_NAME);
    expect(found).toBeDefined();
    expect(VersioningTestHelper.getNameFromItem(found)).toBe(TEST_PAGE_NAME);
  }, 30000);
